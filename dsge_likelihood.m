...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if isfield(DynareOptions,'BLD_Estimation')
% persistent xtc
% 
% if isempty(xtc)
%     xtc = xparam1;
% end
% 
name_ss_mod    = {'xi_p' 'tau_p' 'sigma1_p' 'sigma2_p' 'sigma3_p' 'mub_p' 'mul_p' 'b_p'}';
% 
% xidiff = xparam1 - xtc;
% 
% dd     = [1:length(name_ss_mod)]';
% idx_ss = zeros(length(name_ss_mod),1);
% for ij = 1 : length(name_ss_mod)
% idx_ss(ij) = dd(strcmp(BayesInfo.name,name_ss_mod(ij)));
% end
% 
% if xidiff(idx_ss) ~= 0 
% 
if sum(ismember(BayesInfo.name,name_ss_mod))>0
try
Model.params = BL_param_set(Model);
catch lasterr
%         display(lasterr)
    fval = Inf;
    info(1) = 47;
